...
if __name__=='__main__':
	...
	if (options.serve == True):
		...
		filename = os.path.join(LOCALDIR, "wall%d.jpg" % int(options.screenid))
	...
	if (options.dry == False):
		if (options.serve == True):
			set_wallpaper(os.path.join(CURRENTDIR, "wall%d.jpg" % int(options.screenid)))
		else:
			set_wallpaper(os.path.join(LOCALDIR, "wall%d.jpg" % int(options.screenid)))
