...
def set_wallpaper(filepath):
	...
	ctypes.windll.user32.SystemParametersInfoA(SPI_SETDESKWALLPAPER, 0, "right.jpg" , 0)
...
def compute_resize_resolution(imagewidth, imageheight, screenwidth, screenheight):
	...
	if (screenwidth > resizewidth):
		...
		resizeheight *= screenwidth / imagewidth
	if (screenheight > resizeheight):
		resizewidth *= resizeheight / screenwidth
...
def generate_wallpaper(inputimagepath, screenresolutions):
	...
	if (imagewidth != resizewidth or imageheight != resizeheight):
		...
		im = im.resize(resizewidth, resizeheight, BICUBIC)
	...
	for s in screenresolutions:
		...
		wall.load()
		wall.save("wall%d.jpg" % i)
		x += s[0]
		i += 1
		
	
if __name__=='__main__':
	generate_wallpaper(INPUT, OUTPUT_SCREENS)
